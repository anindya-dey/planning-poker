...
export default function Home() {
    ...
    return (
        <div className="flex justify-center items-center h-screen selection:bg-indigo-600 selection:text-white">
            <div className="w-5/12 px-6 py-16 border border-gray-200 rounded-lg shadow-lg shadow-indigo-100">
                <div className="w-full flex flex-col space-y-10 justify-center">
                    ...
                    <div className="flex space-x-2 w-full">
                        <input type="text" className="py-2 w-3/5 rounded border border-gray-300 placeholder:text-indigo-300 text-indigo-600 focus:ring-indigo-600" placeholder="Room Id" value={roomId} onChange={() => setRoomId(roomId) }/>
                        <Link className="w-2/5" to={`/room/${roomId}`}>
                            <div className="px-6 py-2 bg-indigo-600 text-white text-center rounded hover:bg-indigo-800 hover:cursor-pointer transition-all ease-in-out">Join a room</div>
                        </Link>
                    </div>
